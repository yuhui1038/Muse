...
def decode_one(mucodec, path):
    ...
    # codes = code.unsqueeze(0).unsqueeze(0)
...
if __name__=="__main__":
    ...
    # filelist = []
    # root_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_segs")
    # save_dir = "xxx/reconstructed/segs"
    # for f in tqdm([os.path.join(root_dir, f) for f in os.listdir(root_dir) if '.pt' in f]):
    #     code = torch.load(f)
    #     codes = code.unsqueeze(0).unsqueeze(0)
    ...
    #     save_path = os.path.basename(f)[:-2] + "wav"
    #     torchaudio.save(os.path.join(save_dir, save_path), wave.detach().cpu(), 48000)